...
<objetivo>
# Carta: operaciones — Nemawashi
</objetivo>

<metodo>

## La suite de cierre no es tuya, pero sí que pueda correr
...
## El andon, y por qué corre sólo un contrato

`.github/workflows/andon.yml` se jala en cada *pull request* y en cada push a `main`. Corre **una
sola cosa**: `src/contratos/sin-dato-personal.test.ts`.
...
**Consecuencia que hay que tener presente:** lo que el andon **no** cubre —los tipos, la compilación,
la app abriendo— hoy sólo se verifica en la máquina de quien trabaja. Si eso se va a cerrar, se cierra
con un runner que sí pueda abrir una ventana, no bajándole la exigencia al andon.
...
Electron se descargó completo y se descomprimió a medias **sin error visible**: la instalación se dio
por buena y la app no arrancaba.
...
## Cómo se comprueba que arranca en otra máquina
...
## Los procesos que la sesión necesita al lado
